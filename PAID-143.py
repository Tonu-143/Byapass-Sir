...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}TONU123{N}")
...
class __chigoue__:
 ...
 #PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[TONU]","\x1b[1;92m[TONU]","\x1b[1;92m[TONU]","\x1b[1;92m[TONU]","\x1b[1;92m[TONU]","\x1b[1;92m[TONU]","\x1b[1;92m[TONU]"])
               ...
               # ua2 = random.choice(agen2)
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[TONU-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
